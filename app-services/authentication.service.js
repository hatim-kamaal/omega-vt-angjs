	...
	function AuthenticationService($http, $cookieStore, $rootScope, $timeout,
		...
		function Login(username, password, callback) {
			...
			// });
				...
				$http.get('http://localhost/omega-vt-angjs/services/member/get_by_username/'+username).success(function(response) {
				//var data = response.data;
				if( response.data.length == 1 ) {
					var data = response.data[0];
					var response;
					if(data.member_code === password) {
			...
			}).error( function(response){alert("Erro");callback(response);} );
		}
